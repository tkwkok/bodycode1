...
interface GamifiedReminderProps {
    supplements: string[];
}

type TimeOfDay = '아침' | '점심' | '저녁';
const times: TimeOfDay[] = ['아침', '점심', '저녁'];

interface State {
    reminders: { [supplement: string]: TimeOfDay[] };
    completion: { [supplement: string]: { [dayTime: string]: boolean } };
    points: number;
}

type Action =
    | { type: 'TOGGLE_REMINDER'; supplement: string; time: TimeOfDay }
    | { type: 'TOGGLE_COMPLETION'; supplement: string; time: TimeOfDay }
...
const GamifiedReminder: React.FC<GamifiedReminderProps> = ({ supplements }) => {
                    ...
                    {supplements.map(supplement => (
                        <div key={supplement} className="p-4 bg-slate-50 dark:bg-slate-800/70 rounded-lg">
                            ...
                            <div className="mt-3 flex items-center gap-2 flex-wrap">
                                {times.map(time => {
                                    const isReminderSet = reminders[supplement]?.includes(time);
